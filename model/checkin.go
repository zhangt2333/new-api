...
func userCheckinWithTransaction(checkin *Checkin, userId int, quotaAwarded int) (*Checkin, error) {
	err := DB.Transaction(func(tx *gorm.DB) error {
		// START custom quota ceiling change: lock the user and clamp the check-in reward.
		var user User
		...
		// END custom quota ceiling change: transactional check-in reward is capped.
		...
		// 步骤2: 在事务中增加用户额度
		// START custom quota ceiling change: credit only the clamped reward.
		if err := tx.Model(&User{}).Where("id = ?", userId).
			Update("quota", gorm.Expr("quota + ?", actualAward)).Error; err != nil {
			return errors.New("签到失败：更新额度出错")
		}
		// END custom quota ceiling change: clamped reward credited.
	...
	// 事务成功后，异步更新缓存
	// START custom quota ceiling change: keep cached quota aligned with the clamped reward.
	go func() {
		_ = cacheIncrUserQuota(userId, int64(checkin.QuotaAwarded))
	}()
	// END custom quota ceiling change: cache update uses the actual reward.
...
func userCheckinWithoutTransaction(checkin *Checkin, userId int, quotaAwarded int) (*Checkin, error) {
	// START custom quota ceiling change: clamp SQLite check-in rewards before writing.
	...
	checkin.QuotaAwarded = actualAward
	// END custom quota ceiling change: SQLite check-in reward is capped.
	...
	// START custom quota ceiling change: credit only the clamped SQLite reward.
	if err := IncreaseUserQuota(userId, actualAward, true); err != nil {
		// 如果增加额度失败，需要回滚签到记录
		DB.Delete(checkin)
		return nil, errors.New("签到失败：更新额度出错")
	}
	// END custom quota ceiling change: clamped SQLite reward credited.
